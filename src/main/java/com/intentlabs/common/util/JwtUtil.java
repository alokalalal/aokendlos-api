...
public class JwtUtil {
	 ...
	 * @throws EndlosAPIException
	 */
	public static Claims extractAllClaims(String token) throws EndlosAPIException {
		try {
			return Jwts.parser().setSigningKey(SystemSettingModel.getSecretKeyForGenerateJWTToken())
					.parseClaimsJws(token).getBody();
		} catch (SignatureException signatureException) {
			throw new EndlosAPIException(ResponseCode.INVALID_JSON_TOKEN.getCode(),
	...
	public static String generateAccessToken(String subject, String requestJsonPayload, JwtTokenModel jwtTokenModel) {
		...
		if (jwtTokenModel != null) {
			...
			} else if (jwtTokenModel.isCaptchaToken()) {
				expiryTimeInMinutes = SystemSettingModel.getCaptchaJwtTokenExpiryTimeInMinutes();
	 ...
	 * @return
	 */
	public static String generateAccessTokenForMachine(String subject, String requestJsonPayload) {
		Map<String, Object> claims = new HashMap<>();
		if (!StringUtils.isBlank(requestJsonPayload)) {
			claims.put(Constant.REQUEST_PAYLOAD, requestJsonPayload);
		}
